...
public class DataTransmitter
{
    ...
    public OMCW omcw;
    private Queue<DataFrame> _frameQueue = [];
    ...
    private const int
        RequestToSend =
            2; // A7 RTS in Schematic   (Request to Send signal, Trigger, to transmit the FIFO contents to the receiver)

    private const int WriteEnable = 3; // A6 /W In Schematic    (Write strobe to insert 16-bits of data into the FIFO)

    private const int
        EmptyFifo = 4; // A5 /EF in Schematic   (Signal to indicate the FIFO is empty and ready for a new frame)

    private const int
        PeriphReset = 17; // A4 /RST in Schematic  (Resets both the FIFO, and DDS chip into initial states)

    private const int
        Direction = 27; // A3 FLDIR in Schematic (FIFO when in reset mode reads this for cascading, or MSB/LSB indication for data direction)
    ...
    public DataTransmitter(OMCW omcw)
    {
        this.omcw = omcw;
    }
